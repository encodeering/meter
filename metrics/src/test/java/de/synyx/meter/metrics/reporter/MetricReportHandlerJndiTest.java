...
public class MetricReportHandlerJndiTest extends MetricReportTestSupport {
    ...
    public void testReporterUrl () throws Exception {
...

        {
            ...
            assertThat (new MetricReportHandlerJndi (jndicontext).select (mediator (handler), provider, URI.create (endpoint)).get (),
        ...
        verify (handler).reporter (any (MetricReportMediator.class), Matchers.eq (provider), Matchers.eq (anyurl));
    }
    ...
    public void testReporterNotFound () throws Exception {
...

        {
            ...
            assertThat (new MetricReportHandlerJndi (jndicontext).select (mediator (handler), provider, URI.create (endpoint)).isPresent (),
        ...
        verify (handler, never ()).reporter (any (MetricReportMediator.class), Matchers.eq (provider), Matchers.eq (anyurl));
    }
    ...
    public void testReporterLookupFail () throws NamingException {
        ...
        assertThat (new MetricReportHandlerJndi (jndicontext).select (mediator (handler (anyurl.getScheme (), reporter)), provider, URI.create (endpoint)).isPresent (),
